...
router.post('/:slug', async (req, res) => {
  ...
  // Rate limiting per IP e slug (sliding window)
  ...
  try {
    ...
    if (!isValid) {
      ...
      await db.query('INSERT INTO access_logs (protected_page_id, ip_hash, success) VALUES ($1, $2, $3)', [pageData.id, ipHash, false]);
    ...
    res.cookie(`auth_${slug}`, accessToken, { httpOnly: true, sameSite: 'lax', maxAge: 24 * 60 * 60 * 1000 });

    db.query('UPDATE protected_pages SET visits_count = visits_count + 1 WHERE id = $1', [pageData.id]).catch(console.error);

    res.json({ success: true, notionUrl: pageData.notionUrl, accessToken, proxyUrl: `/api/p/view/${slug}` });
  } catch (error) {
